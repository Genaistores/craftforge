...
export function HowItWorks() {
  return (
    ...
    >
      ...
      {/* Steps */}
      <div className="mx-auto grid max-w-4xl grid-cols-1 gap-5 sm:grid-cols-3 sm:gap-8">
        {STEPS.map((s) => (
          <div
            key={s.number}
            className="rounded-2xl border border-white/[0.08] bg-[#0F1A2E] p-6 shadow-[0_16px_48px_rgba(2,8,23,0.45)] ring-1 ring-white/[0.04] sm:p-8"
          >
            ...
            <p className="text-base leading-relaxed text-slate-400 sm:text-[15px] sm:leading-[1.7]">
              {s.description}
            </p>
          </div>
        ))}
